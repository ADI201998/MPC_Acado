...
class Behaviour(Node):
	def __init__(self):
		...
		self.obs_pedestrian = np.array([[120.0, -6.0, 1.5707963, 0.0, 0.0]])
		self.sign = -1
	...
	def plot(self, twist, path, kkt):
		...
		for i in range(len(self.goal_p.poses)):
			...
			cruise_speed.append(np.linalg.norm(self.path_v[i*51:i*51 + 51] - 20.0))
		...
		min_cost = np.inf
		for i in range(len(self.goal_p.poses)):
			cost = 0*y_idx[i] + 4*obs_idx[i] + 5*cruise_idx[i] + 2*ang_idx[i]
			if cost<min_cost:
				min_cost = cost
				index = i
