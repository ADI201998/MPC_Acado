...
class Behaviour(Node):
	...
	def get_lane_cons_stop(self):
		...
		#	self.obs[0][3] = 2.0
		if abs(self.agent_pose[0]-self.obs[0][0])<40:
			self.behaviour_event = 3
			self.obs[0][3] = 2.0
		if abs(self.obs[0][0]-self.agent_pose[0])>30:
			self.behaviour_event = 1
			return self.get_lane_cons_overtake()
		if self.agent_pose[0]-self.obs[0][0]>20:
			self.obs[0][0] = self.agent_pose[0]+random.choice([60, 70, 80, 90, 100])
			self.obs[0][1] = -6
			self.obs[0][3] = 0.0
