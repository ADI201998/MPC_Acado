...
class CombinedEnv(Node):
	def __init__(self) -> None:
		...
		self.obstacles_intersection_north = np.array([[202, -22.0, 90*2*np.pi/360, 7.8, 0.0, 8.0, 0],
												[202.0, 28.0, 90*2*np.pi/360, 8.3, 0.0, 8.0, 0],
												[202.0, 68.0, 90*2*np.pi/360, 7.9, 0.0, 8.0, 0],
												#######################
												[206.0, -37.0, 90*2*np.pi/360, 9.8, 0.0, 10.0, 0],
												[206.0, 18.0, 90*2*np.pi/360, 9.9, 0.0, 10.0, 0],
												[206.0, 53.0, 90*2*np.pi/360, 10.3, 0.0, 10.0, 0],
												#######################
												[210.0, -13.0, 90*2*np.pi/360, 12.3, 0.0, 12.0, 0],
												[210.0, 38.0, 90*2*np.pi/360, 11.8, 0.0, 12.0, 0],
												[210.0, 70.0, 90*2*np.pi/360, 11.9, 0.0, 12.0, 0],
												])

		self.obstacles_intersection_south = np.array([[222.0, 0.0, 270*2*np.pi/360, 12.3, 0.0, 12.0, 0],
												[222.0, 40.0, 270*2*np.pi/360, 11.8, 0.0, 12.0, 0],
												[222.0, 80.0, 270*2*np.pi/360, 11.9, 0.0, 12.0, 0],
												#######################
												[218.0, -10.0, 270*2*np.pi/360, 9.8, 0.0, 10.0, 0],
												[218.0, 20.0, 270*2*np.pi/360, 9.9, 0.0, 10.0, 0],
												[218.0, 60.0, 270*2*np.pi/360, 10.3, 0.0, 10.0, 0],
												######################
												[214.0, -40.0, 270*2*np.pi/360, 7.8, 0.0, 8.0, 0],
												[214.0, 0.0, 270*2*np.pi/360, 8.3, 0.0, 8.0, 0],
												[214.0, 30.0, 270*2*np.pi/360, 7.9, 0.0, 8.0, 0],
												])
						
		self.obstacles_right = np.array([[-12.0, 10.0, 0.0*2*np.pi/360, 7.8, 0.0, 8.0, 102.0],
										...
										[90.0, 6.0, 0.0*2*np.pi/360, 10.3, 0.0, 10.0, 106.0],
										#######################
		...
		self.obstacles_left = np.array([[-12.0, -2.0, 180*2*np.pi/360, 7.8, 0.0, 8.0, 114.0],
												[35.0, -2.0, 180*2*np.pi/360, 8.3, 0.0, 8.0, 114.0],
												[80.0, -2.0, 180*2*np.pi/360, 7.9, 0.0, 8.0, 114.0],
												#######################
												[-5.0, -6.0, 180*2*np.pi/360, 9.8, 0.0, 10.0, 118.0],
												[45.0, -6.0, 180*2*np.pi/360, 9.9, 0.0, 10.0, 118.0],
												[90.0, -6.0, 180*2*np.pi/360, 10.3, 0.0, 10.0, 118.0],
												#######################
												[-20.0, -10.0, 180*2*np.pi/360, 12.3, 0.0, 12.0, 122.0],
												[40.0, -10.0, 180*2*np.pi/360, 11.8, 0.0, 12.0, 122.0],
												[100.0, -10.0, 180*2*np.pi/360, 11.9, 0.0, 12.0, 122.0],
												])	
	...
	def send_request(self):
		...
		self.req.start.pose.pose.position.x = self.agent_pose[0]#0.0
	...
	def update_obstacles_normal(self):
		...
		for i in range(self.obstacles_right.shape[0]):
			...
			if self.obstacles_right[i][0] - self.agent_pose[0] < -50.0:
				self.obstacles_right[i][0] = self.obstacles_right[i][0] + 150.0
		...
		for i in range(self.obstacles_left.shape[0]):
			...
			if self.obstacles_left[i][0] - self.agent_pose[0] < -30.0:
				self.obstacles_left[i][0] = self.obstacles_left[i][0] + 130.0
	...
	def plot_agents(self):
		...
		agent = [self.agent_pose[0], self.agent_pose[1], self.lane_radii[0]+2]
		env_x, env_y, env_theta = self.centerline_to_env(agent)
		self.agent_pose_frenet = [env_x, env_y, 0.0]
	...
	def plot(self, twist, path, kkt):
		...
		for i in range(len(self.lane_y)):
			res_obs_x = np.linalg.norm(path_x[i*51:i*51 + 51] - self.nearest_obstacles[:, :51], axis=1)
			res_obs_y = np.linalg.norm(path_y[i*51:i*51 + 51] - self.nearest_obstacles[:, 51:], axis=1)
			res_obs = np.vstack((res_obs_x, res_obs_y))
			res_obs = np.linalg.norm(res_obs, axis=0)
			obs_cost.append(1/np.min(res_obs))
			y_dist.append(np.linalg.norm(path_y[i*51:i*51 + 51] - 14.0))
			kkt_cost.append(kkt[i])
			cruise_speed.append(np.linalg.norm(path_v[i*51:i*51 + 51] - 15.0))
			angular_vel.append(np.linalg.norm(path_w[i*51:i*51 + 51] - 0.0))
		...
		for i in range(len(self.lane_y)):
			...
			env_theta = []
			if i == index:
				twist.linear.x = path_v[i*51+1]
				twist.angular.z = path_w[i*51+1]
				for p in range(51):
					agent = [path_x[i*51 + p], path_y[i*51 + p], path_theta[i*51 + p], self.lane_radii[0]+2]
				...
				# twist.linear.x = path_v[i*51+1]
				# twist.angular.z = np.diff(np.array(env_theta))[1]/0.1
			else:
				for p in range(51):
					agent = [path_x[i*51 + p], path_y[i*51 + p], self.lane_radii[0]+2]
					x_env, y_env, theta_env = self.centerline_to_env(agent)
					env_x.append(x_env)
					env_y.append(y_env)
