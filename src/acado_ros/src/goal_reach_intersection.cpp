...
GoalReach::GoalReach(): Node("acado_goal_reach_srv"), count_(0)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        acadoVariables.W[NY*NY*i + (NY+1)*5] = 0.5;
          ...
          const std::shared_ptr<acado_msgs::srv::GetVelocityCmd::Response>      response)
{
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        {
            acadoVariables.od[i * NOD + 10] = pa.poses[1].position.x;
            acadoVariables.od[i * NOD + 11] = pa.poses[1].position.y;
            acadoVariables.od[i * NOD + 12] = pa.poses[1].position.z;
            if(i==N)
                continue;
            ...
            /*RCLCPP_INFO(this->get_logger(), "INTERSECTION...... = %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f", acadoVariables.od[i * NOD + 10],
                                    acadoVariables.od[i * NOD + 11], acadoVariables.od[i * NOD + 12],
