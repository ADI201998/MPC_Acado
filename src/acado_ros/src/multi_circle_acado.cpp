...
GoalReach::GoalReach(): Node("acado_goal_reach_srv"), count_(0)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        /*acadoVariables.W[NY*NY*i + (NY+1)*6] = 500;
        acadoVariables.W[NY*NY*i + (NY+1)*7] = 500;
        acadoVariables.W[NY*NY*i + (NY+1)*8] = 500;
        acadoVariables.W[NY*NY*i + (NY+1)*9] = 500;
        acadoVariables.W[NY*NY*i + (NY+1)*10] = 500;*/
    }

    acadoVariables.WN[(NYN+1)*0] = 10;
	acadoVariables.WN[(NYN+1)*1] = 10;
	acadoVariables.WN[(NYN+1)*2] = 10;


    for (int i = 0; i < N; i++)
    {
        acadoVariables.lbAValues[i*4+0] = -1e12;    //x
        acadoVariables.lbAValues[i*4+1] = -10.9;     //y
        acadoVariables.lbAValues[i*4+2] = -5;       //v
        acadoVariables.lbAValues[i*4+3] = -2;       //w

        acadoVariables.ubAValues[i*4+0] = 1e12;     //x
        acadoVariables.ubAValues[i*4+1] = 10.9;     //y
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < 45; j++)
        {
            acadoVariables.lbAValues[i*45+N*4+j] = 2.01;
            acadoVariables.ubAValues[i*45+N*4+j] = 1e12;
        }
    }
          ...
          const std::shared_ptr<acado_msgs::srv::GetVelocityCmd::Response>      response)
{
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        acadoVariables.od[i * NOD + 3] = odom_arr.odom[1].pose.pose.position.x + odom_arr.odom[1].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 4] = odom_arr.odom[1].pose.pose.position.y + odom_arr.odom[1].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 6] = odom_arr.odom[2].pose.pose.position.x + odom_arr.odom[2].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 7] = odom_arr.odom[2].pose.pose.position.y + odom_arr.odom[2].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 9] = odom_arr.odom[3].pose.pose.position.x + odom_arr.odom[3].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 10] = odom_arr.odom[3].pose.pose.position.y + odom_arr.odom[3].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 12] = odom_arr.odom[4].pose.pose.position.x + odom_arr.odom[4].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 13] = odom_arr.odom[4].pose.pose.position.y + odom_arr.odom[4].twist.twist.linear.y*0.1*i;

        acadoVariables.od[i * NOD + 15] = 0;
        acadoVariables.od[i * NOD + 16] = 1;
        acadoVariables.od[i * NOD + 17] = -request->goal.position.y;
    }
    acado_tic( &t );
