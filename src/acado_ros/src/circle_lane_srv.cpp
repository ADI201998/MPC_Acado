...
GoalReach::GoalReach(): Node("acado_circle_lane_srv"), count_(0)
{
    ...
    for (i = 0; i < (N + 1); ++i)  
    {
        ...
        acadoVariables.x[ NX * i + 1 ] = -106.0; // y
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        acadoVariables.od[i * NOD + 11] = 0;
        acadoVariables.od[i * NOD + 12] = 106;

        acadoVariables.od[i * NOD + 13] = 0;
        acadoVariables.od[i * NOD + 14] = 0;
        acadoVariables.od[i * NOD + 15] = 106.5;

        acadoVariables.od[i * NOD + 16] = 0;
        acadoVariables.od[i * NOD + 17] = 0;
        acadoVariables.od[i * NOD + 18] = 101.5;
    }
    for (int i = 0; i < N; i++)
    {
    ...
    for (int i = 0; i < N; i++)
    {
        acadoVariables.lbAValues[i*2+N*2+0] = -1e12;
        acadoVariables.lbAValues[i*2+N*2+1] = 0;
          ...
          const std::shared_ptr<acado_msgs::srv::GetVelocityCmd::Response>      response)
{
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        acadoVariables.od[i * NOD + 2] = odom_arr.odom[1].pose.pose.position.x + odom_arr.odom[1].twist.twist.linear.x*cos(theta1)*0.1*i;
        acadoVariables.od[i * NOD + 3] = odom_arr.odom[1].pose.pose.position.y + odom_arr.odom[1].twist.twist.linear.x*sin(theta1)*0.1*i;

        double theta2 = odom_arr.odom[2].pose.pose.orientation.z + odom_arr.odom[2].twist.twist.angular.z*0.1*i;
        acadoVariables.od[i * NOD + 4] = odom_arr.odom[2].pose.pose.position.x + odom_arr.odom[2].twist.twist.linear.x*cos(theta2)*0.1*i;
        acadoVariables.od[i * NOD + 5] = odom_arr.odom[2].pose.pose.position.y + odom_arr.odom[2].twist.twist.linear.x*sin(theta2)*0.1*i;

        double theta3 = odom_arr.odom[3].pose.pose.orientation.z + odom_arr.odom[3].twist.twist.angular.z*0.1*i;
        acadoVariables.od[i * NOD + 6] = odom_arr.odom[3].pose.pose.position.x + odom_arr.odom[3].twist.twist.linear.x*cos(theta3)*0.1*i;
        acadoVariables.od[i * NOD + 7] = odom_arr.odom[3].pose.pose.position.y + odom_arr.odom[3].twist.twist.linear.x*sin(theta3)*0.1*i;

        double theta4 = odom_arr.odom[4].pose.pose.orientation.z + odom_arr.odom[4].twist.twist.angular.z*0.1*i;
        acadoVariables.od[i * NOD + 8] = odom_arr.odom[4].pose.pose.position.x + odom_arr.odom[4].twist.twist.linear.x*cos(theta4)*0.1*i;
        acadoVariables.od[i * NOD + 9] = odom_arr.odom[4].pose.pose.position.y + odom_arr.odom[4].twist.twist.linear.x*sin(theta4)*0.1*i;

        acadoVariables.od[i * NOD + 10] = 0;
        acadoVariables.od[i * NOD + 11] = 0;
        acadoVariables.od[i * NOD + 12] = request->goal.position.z;
    }
    acado_tic( &t );
