...
class GoalReach : public rclcpp::Node
{
  ...
  private:
    ...
    std::vector<geometry_msgs::msg::Twist> twist_vec;
    bool high_cost;
    int index;
...
GoalReach::GoalReach(): Node("acado_goal_reach_srv"), count_(0)
{
    ...
    for (int i = 0; i < N; i++)
    {
        acadoVariables.W[NY*NY*i + (NY+1)*0] = 0.0075;
        acadoVariables.W[NY*NY*i + (NY+1)*1] = 0.0075;
        acadoVariables.W[NY*NY*i + (NY+1)*2] = 10;
        //acadoVariables.W[NY*NY*i + (NY+1)*2] = 500;  # Lane change
        //acadoVariables.W[NY*NY*i + (NY+1)*3] = 500;
        acadoVariables.W[NY*NY*i + (NY+1)*3] = 80;
        acadoVariables.W[NY*NY*i + (NY+1)*4] = 80;
        acadoVariables.W[NY*NY*i + (NY+1)*5] = 0.25;
        /*acadoVariables.W[NY*NY*i + (NY+1)*6] = 0.75;
        acadoVariables.W[NY*NY*i + (NY+1)*7] = 0.75;
        acadoVariables.W[NY*NY*i + (NY+1)*8] = 0.75;
        acadoVariables.W[NY*NY*i + (NY+1)*9] = 0.75;
        acadoVariables.W[NY*NY*i + (NY+1)*10] = 0.75;*/
    }
          ...
          const std::shared_ptr<acado_msgs::srv::GetVelocityCmd::Response>      response)
{
    ...
    if(!high_cost)
    {
        ...
        for (i = 0; i < (N + 1); ++i)  
        {
            ...
            cmd_vel.linear.x = acadoVariables.x[NX*i + 3];
            cmd_vel.angular.z = acadoVariables.x[NX*i + 4];
            twist_vec.push_back(cmd_vel);
    ...
    else
    {
        ...
        {
            geometry_msgs::msg::Pose pose;
            pose.position.x = local_path.poses[i].position.x;
            pose.position.y = local_path.poses[i].position.y;
            path_val.poses.push_back(pose);
            //RCLCPP_INFO(this->get_logger(), "ACC_lin: %.2f, ACC_ang: %.2f", acadoVariables.u[ NU * i + 0], acadoVariables.x[ NU * i + 1 ]);
        }
    }
    ...
    {
        cmd_vel.linear.x = twist_vec[index].linear.x;
        cmd_vel.angular.z = twist_vec[index].angular.z;
        index++;
    }
    /*acadoVariables.x0[ 0 ] = acadoVariables.x[NX + 0]; // x
    acadoVariables.x0[ 1 ] = acadoVariables.x[NX + 1]; // y
