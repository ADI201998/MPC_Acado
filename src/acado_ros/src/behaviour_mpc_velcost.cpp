...
GoalReach::GoalReach(): Node("acado_circle_lane_srv"), count_(0)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        acadoVariables.ubAValues[i*2+0] = 25;       //v
          ...
          const std::shared_ptr<acado_msgs::srv::GetControls::Response>      response)
{
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        if (i >= lane_cons.poses[0].position.x)
        {
            ...
            if(lane_cons.poses[7].position.x == 4.0)
            {
                acadoVariables.y[ NY * i + 2 ] = lane_cons.poses[7].position.y;
                acadoVariables.W[NY*NY*i + (NY+1)*2] = lane_cons.poses[7].position.z;
            }
            else if(lane_cons.poses[7].position.x == 5.0)
            {
                acadoVariables.y[ NY * i + 2 ] = lane_cons.poses[7].position.y;
                acadoVariables.W[NY*NY*i + (NY+1)*2] = lane_cons.poses[7].position.z;
            }
            else if(lane_cons.poses[7].position.x == 1.0)
            {
                acadoVariables.y[ NY * i + 2 ] = lane_cons.poses[7].position.y;
                acadoVariables.W[NY*NY*i + (NY+1)*2] = lane_cons.poses[7].position.z;
            }
