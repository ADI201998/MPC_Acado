...
GoalReach::GoalReach(): Node("acado_goal_reach_srv"), count_(0)
{
    ...
    for (i = 0; i < N; ++i)  
    {
        ...
        acadoVariables.y[ NY * i + 15 ] = 0.0; 	//  obs
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        acadoVariables.od[i * NOD + 8] = 18.0*1000;
        acadoVariables.od[i * NOD + 9] = -4.0*1000;
        acadoVariables.od[i * NOD + 10] = -4.0*1000;
        acadoVariables.od[i * NOD + 11] = 18.0*1000;
        acadoVariables.od[i * NOD + 12] = -4.0*1000;
        acadoVariables.od[i * NOD + 13] = 18.0*1000;
        acadoVariables.od[i * NOD + 14] = -4.0*1000;
        acadoVariables.od[i * NOD + 15] = -4.0*1000;
        acadoVariables.od[i * NOD + 16] = -4.0*1000;
        acadoVariables.od[i * NOD + 17] = -4.0*1000;
        acadoVariables.od[i * NOD + 18] = -4.0*1000;
        acadoVariables.od[i * NOD + 19] = -4.0*1000;*/
        for(int j = 0; j<10; j++)
        {
            acadoVariables.od[i * NOD + j*2+0] = -4.0*1000;
            acadoVariables.od[i * NOD + j*2+1] = -4.0*1000;
        }

        acadoVariables.od[i * NOD + 20] = 0;
        acadoVariables.od[i * NOD + 21] = 1;
        acadoVariables.od[i * NOD + 22] = 6;
    }
    for (int i = 0; i < N; i++)
    {
        acadoVariables.W[NY*NY*i + (NY+1)*0] = 0.1;
        acadoVariables.W[NY*NY*i + (NY+1)*1] = 0.1;
        ...
        acadoVariables.W[NY*NY*i + (NY+1)*4] = 100;
        acadoVariables.W[NY*NY*i + (NY+1)*5] = 5.0;
        acadoVariables.W[NY*NY*i + (NY+1)*6] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*7] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*8] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*9] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*10] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*11] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*12] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*13] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*14] = 2000.0;
        acadoVariables.W[NY*NY*i + (NY+1)*15] = 2000.0;
    }

    acadoVariables.WN[(NYN+1)*0] = 0;
	acadoVariables.WN[(NYN+1)*1] = 0;
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        acadoVariables.ubAValues[i*4+2] = 20;       //v
        acadoVariables.ubAValues[i*4+3] = 2;        //w
    }
    /*for (int i = 0; i < N; i++)
    {
        acadoVariables.lbAValues[i*5+N*4+0] = 2.001;
        acadoVariables.lbAValues[i*5+N*4+1] = 2.001;
        acadoVariables.lbAValues[i*5+N*4+2] = 2.001;
        acadoVariables.lbAValues[i*5+N*4+3] = 2.001;
        acadoVariables.lbAValues[i*5+N*4+4] = 2.001;
          ...
          const std::shared_ptr<acado_msgs::srv::GetVelocityCmd::Response>      response)
{
    ...
    /*RCLCPP_INFO(this->get_logger(), "INTERSECTION...... = %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f", pa.poses[0].position.x,
                                    pa.poses[1].position.x, pa.poses[1].position.y, pa.poses[1].position.z,
                                    pa.poses[2].position.x, pa.poses[2].position.y,
                                    pa.poses[3].position.x, pa.poses[3].position.y);
    exit(1);*/
    ...
    for (int i = 0; i < (N + 1); ++i)
    {
        ...
        /*acadoVariables.od[i * NOD + 10] = odom_arr.odom[5].pose.pose.position.x + odom_arr.odom[5].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 11] = odom_arr.odom[5].pose.pose.position.y + odom_arr.odom[5].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 12] = odom_arr.odom[6].pose.pose.position.x + odom_arr.odom[6].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 13] = odom_arr.odom[6].pose.pose.position.y + odom_arr.odom[6].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 14] = odom_arr.odom[7].pose.pose.position.x + odom_arr.odom[7].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 15] = odom_arr.odom[7].pose.pose.position.y + odom_arr.odom[7].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 16] = odom_arr.odom[8].pose.pose.position.x + odom_arr.odom[8].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 17] = odom_arr.odom[8].pose.pose.position.y + odom_arr.odom[8].twist.twist.linear.y*0.1*i;
        acadoVariables.od[i * NOD + 18] = odom_arr.odom[9].pose.pose.position.x + odom_arr.odom[9].twist.twist.linear.x*0.1*i;
        acadoVariables.od[i * NOD + 19] = odom_arr.odom[9].pose.pose.position.y + odom_arr.odom[9].twist.twist.linear.y*0.1*i;*/
        for(int j = 0; j< odom_arr.odom.size(); j++)
        {
            acadoVariables.od[i * NOD + j*2+0] = odom_arr.odom[j].pose.pose.position.x + odom_arr.odom[j].twist.twist.linear.x*0.1*i;
            acadoVariables.od[i * NOD + j*2+1] = odom_arr.odom[j].pose.pose.position.y + odom_arr.odom[j].twist.twist.linear.y*0.1*i;
        }
        /*RCLCPP_INFO(this->get_logger(), "INTERSECTION...... = %.2f, %.2f, \n%.2f, %.2f, n%.2f, %.2f, \n%.2f, %.2f, \n%.2f, %.2f \n-------------------\n", acadoVariables.od[i * NOD + 0],
                                    acadoVariables.od[i * NOD + 1], acadoVariables.od[i * NOD + 2],
                                    acadoVariables.od[i * NOD + 3], acadoVariables.od[i * NOD + 4],
                                    acadoVariables.od[i * NOD + 5], acadoVariables.od[i * NOD + 6],
                                    acadoVariables.od[i * NOD + 7], acadoVariables.od[i * NOD + 8],
                                    acadoVariables.od[i * NOD + 9]);*/
        
        if(i>=(int)pa.poses[0].position.x)
        {
            acadoVariables.od[i * NOD + 20] = pa.poses[1].position.x;
            acadoVariables.od[i * NOD + 21] = pa.poses[1].position.y;
            acadoVariables.od[i * NOD + 22] = pa.poses[1].position.z;
            if(i==N)
                continue;

            acadoVariables.lbAValues[i*4+0] = pa.poses[2].position.x;    //x
            acadoVariables.lbAValues[i*4+1] = pa.poses[2].position.y;     //y

            acadoVariables.ubAValues[i*4+0] = pa.poses[3].position.x;     //x
            acadoVariables.ubAValues[i*4+1] = pa.poses[3].position.y;     //y

            RCLCPP_INFO(this->get_logger(), "INTERSECTION...... = %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f", acadoVariables.od[i * NOD + 20],
                                    acadoVariables.od[i * NOD + 21], acadoVariables.od[i * NOD + 22],
                                    acadoVariables.lbAValues[i*4+0], acadoVariables.lbAValues[i*4+1],
                                    acadoVariables.ubAValues[i*4+0], acadoVariables.ubAValues[i*4+1]);
        }
    }
    acado_tic( &t );
...
void GoalReach::check_term_dist(double dist)
{
    ...
    else
    {
        ...
        for (i = 0; i < N; ++i)  
        {
            acadoVariables.y[ NY * i + 2 ] = 15.0;	// theta	
        }
    }
}
